...
.PHONY: all
all: fmt bindata lint vendor | $(BASE) ; $(info $(M) building executable…) @ ## Build program binary
	$Q cd $(BASE) && GOOS=$(OS) GOARCH=$(ARCH) $(GO) build \
		-tags release \
		-ldflags '-X $(PACKAGE)/cmd.Version=$(VERSION) -X $(PACKAGE)/cmd.BuildDate=$(DATE)' \
		-o bin/$(PACKAGE) main.go bindata.go
...
$(BIN)/go-bindata: | $(BASE) ; $(info $(M) building go-bindata…)
...
bindata: vendor | $(BASE) $(GOBINDATA) ; $(info $(M) running go-bindata…)
	$Q cd $(BASE) && $(GOBINDATA) resources/
...
.PHONY: compress
compress: ; $(info $(M) compressing…)	@ ## Compress executable
	@strip bin/*
	@upx bin/*
